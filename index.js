...
async function syncWithEcowitt(forceWrite = false) {
    ...
    try {
        ...
        try {
            ...
            if (dateCheck.rows.length > 0) {
                ...
                if (oldestDate !== todayIST) {
                    await pool.query(`INSERT INTO daily_max_records (record_date, max_temp_c, min_temp_c, max_wind_kmh, total_rain_mm) SELECT $1, MAX((temp_f - 32) * 5/9), MIN((temp_min_f - 32) * 5/9), MAX(wind_speed_mph * 1.60934), MAX(daily_rain_in * 25.4) FROM weather_history WHERE (time AT TIME ZONE 'Asia/Kolkata')::date = $1::date;`, [oldestDate]);
        ...
        state.cachedData = {
            ...
            rain: { total: parseFloat((d.rainfall.daily.value * 25.4).toFixed(1)), weekly: parseFloat((d.rainfall.weekly.value * 25.4).toFixed(1)), monthly: parseFloat((d.rainfall.monthly.value * 25.4).toFixed(1)), yearly: parseFloat((d.rainfall.yearly.value * 25.4).toFixed(1)), rate: liveRainRate, maxR: mx_r, maxRTime: mx_r_t },
...
<body>
    ...
    function animateWind() {
        ...
        ctxW.strokeStyle = document.body.classList.contains('is-night') ? 'rgba(255, 255, 255, 0.12)' : 'rgba(2, 132, 199, 0.07)';
        ctxW.lineWidth = 1.1;
        ctxW.lineCap = 'round';
        ctxW.beginPath();
        particles.forEach(p => {
