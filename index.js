...
// Persistence for Daily Highs/Lows
...
// --- CORE LOGIC: PERSISTENCE (UNTOUCHED) ---
...
// --- CORE LOGIC: CALCULATIONS (UNTOUCHED) ---
const getCard = (a) => {
...
// --- MAIN SYNC ENGINE (UNTOUCHED LOGIC) ---
async function syncWithEcowitt() {
    ...
    try {
        ...
        // --- IMPROVED DAVIS-STYLE RAIN LOGIC (UNTOUCHED) ---
        let instantRR = 0;
        if (state.todayHistory.length > 0) {
            ...
            if (pastRecord && dailyRain > pastRecord.rainTotal) {
                ...
                // If no rain tips in last 2.5 mins, force rate to 0
        ...
        // Trends & Pressure (3-hour comparison)
        ...
        state.todayHistory.push({ 
            ...
            rain: instantRR, rainTotal: dailyRain, press: pressure, 
...
<head>
    ...
    <style>
        :root { 
            --bg: #0f172a; /* LIGHTER DARK BACKGROUND */
            --card: rgba(30, 41, 59, 0.7); /* GLASS CARD */
            --accent: #38bdf8; --max-t: #fb7185; --min-t: #60a5fa; 
            --wind: #fbbf24; --rain: #818cf8; --border: rgba(255, 255, 255, 0.1);
        }
        * { box-sizing: border-box; -webkit-font-smoothing: antialiased; }
        body { 
            margin: 0; font-family: 'Inter', system-ui, sans-serif; 
            background: linear-gradient(180deg, var(--bg) 0%, #020617 100%);
            color: #f8fafc; padding: 20px 24px; min-height: 100vh;
        }
        .container { width: 100%; max-width: 1200px; margin: 0 auto; }
        
        /* HEADER REALIGNMENT */
        .header { margin-bottom: 24px; text-align: center; display: flex; flex-direction: column; align-items: center; }
        ...
        .dot { width: 8px; height: 8px; background: #22c55e; border-radius: 50%; animation: pulse 2.5s infinite; }
        @keyframes pulse { 0%, 100% { opacity: 1; } 50% { opacity: 0.3; } }
        .live-text { font-family: 'JetBrains Mono', monospace; font-size: 11px; font-weight: 800; color: #22c55e; letter-spacing: 1px; }
        .timestamp { font-family: 'JetBrains Mono', monospace; font-size: 11px; color: #64748b; }

        /* INCREASED DENSITY GRID */
        .grid-system { display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 16px; margin-bottom: 16px; }

        .card, .graph-card { 
            background: var(--card); padding: 24px; border-radius: 24px; 
            border: 1px solid var(--border); position: relative; width: 100%;
            backdrop-filter: blur(12px); box-shadow: 0 15px 25px -5px rgba(0, 0, 0, 0.2);
            transition: all 0.3s ease;
        }
        .card:hover { transform: translateY(-4px); border-color: rgba(255, 255, 255, 0.2); }

        .label { color: #94a3b8; font-size: 11px; font-weight: 800; text-transform: uppercase; letter-spacing: 1.5px; margin-bottom: 10px; }
        
        /* CONDENSED VALUES */
        .main-val { font-size: 48px; font-weight: 900; margin: 0; display: flex; align-items: baseline; letter-spacing: -2px; }
        .unit { font-size: 20px; font-weight: 600; color: #64748b; margin-left: 6px; letter-spacing: 0; }
        .minor-line { font-size: 15px; font-weight: 700; margin-top: 4px; display: flex; align-items: center; gap: 8px; color: var(--accent); }
        .trend-badge { font-size: 13px; font-weight: 700; display: inline-flex; align-items: center; gap: 6px; padding: 4px 10px; background: rgba(255,255,255,0.03); border-radius: 8px; }

        .sub-box-4 { 
            display: grid; grid-template-columns: 1fr 1fr; gap: 10px; 
            margin-top: 20px; padding-top: 16px; border-top: 1px solid rgba(255, 255, 255, 0.08); 
        }
        .badge { padding: 12px; border-radius: 16px; background: rgba(15, 23, 42, 0.4); display: flex; flex-direction: column; gap: 4px; }
        ...
        .compass-ui { position: absolute; top: 24px; right: 24px; width: 44px; height: 44px; border: 2.5px solid rgba(255,255,255,0.05); border-radius: 50%; display: flex; align-items: center; justify-content: center; background: rgba(0,0,0,0.3); }
        #needle { width: 4px; height: 30px; background: linear-gradient(to bottom, var(--max-t) 50%, #fff 50%); clip-path: polygon(50% 0%, 100% 100%, 50% 85%, 0% 100%); transition: transform 1.5s cubic-bezier(0.34, 1.56, 0.64, 1); }

        .graph-card { height: 300px; padding: 20px; }

        @media (max-width: 768px) { body { padding: 16px; } .header h1 { font-size: 24px; } .main-val { font-size: 40px; } .grid-system { grid-template-columns: 1fr; } }
    </style>
</head>
<body>
    <div class="container">
        <div class="header">
            ...
            <div class="live-container">
                <div class="dot"></div><span class="live-text">LIVE DATA</span><span class="timestamp" id="ts">--:--:--</span>
            </div>
        </div>
        ...
        <div class="grid-system">
            <div class="card">
                ...
                <div class="minor-line">RealFeel: <span id="rf">--</span>°C</div>
            ...
            <div class="card">
                ...
                <div class="minor-line" id="p_desc" style="color:#64748b">Stable Barometer</div>
                <div class="sub-box-4">
            ...
            <div class="card">
                ...
                <div class="minor-line" style="color:var(--rain)">
                    <span id="rr_main">Rate: --</span>
                </div>
                <div class="sub-box-4">
                    <div class="badge"><span class="badge-label">Today Max</span><span id="mr" class="badge-val" style="color:var(--rain)">--</span></div>
                    <div class="badge"><span class="badge-label">Status</span><span id="rain_status" class="badge-val">--</span></div>
                </div>
            </div>
        </div>

        <div class="grid-system" style="grid-template-columns: repeat(auto-fit, minmax(400px, 1fr));">
    ...
    <script>
        ...
        function setupChart(id, label, col, minZero = false) {
            ...
            return new Chart(ctx, {
                ...
                options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: true, labels: { color: '#f8fafc', font: { weight: '700', size: 12 } } } },
                    scales: { x: { display: false }, y: { beginAtZero: minZero, ticks: { color: '#64748b', font: { size: 10 } }, grid: { color: 'rgba(255,255,255,0.03)' } } }
                }
            });
        }
        ...
        async function update() {
            try {
                ...
                document.getElementById('rain_status').style.color = d.rain.rate > 0 ? 'var(--rain)' : '#f1f5f9';
                ...
                if (!charts.cT) {
                    charts.cT = setupChart('cT', 'Temperature Trend', '#38bdf8');
                    charts.cH = setupChart('cH', 'Humidity Trend', '#10b981', true);
                    charts.cW = setupChart('cW', 'Wind Dynamics', '#fbbf24', true);
                    charts.cR = setupChart('cR', 'Rain Intensity', '#818cf8', true);
                }
