...
// --- VERCEL COMPATIBLE FETCH FIX ---
// This uses the built-in fetch (Node 18+) or falls back to node-fetch safely
const fetch = (...args) => {
...
<head>
    ...
    <style>
        ...
        .theme-toggle { background: var(--card); border: 1px solid var(--border); padding: 4px; border-radius: 12px; display: flex; gap: 4px; }
        .theme-btn { padding: 6px 10px; border-radius: 8px; font-size: 11px; font-weight: 700; cursor: pointer; color: var(--muted); }
        ...
        .card { background: var(--card); padding: 28px; border-radius: 32px; border: 1px solid var(--border); backdrop-filter: blur(20px); box-shadow: var(--glow); position: relative; }
        .main-val { font-size: 56px; font-weight: 900; margin: 0; letter-spacing: -2px; line-height: 1.1; }
        ...
        .tab-btn { background: var(--card); border: 1px solid var(--border); padding: 12px 24px; border-radius: 16px; color: var(--text); font-weight: 700; cursor: pointer; }
        .tab-btn.active { background: var(--accent); color: white; }
        .summary-table { width: 100%; border-collapse: collapse; min-width: 600px; }
        .summary-table th { padding: 16px; background: var(--badge); text-align: left; font-size: 11px; color: var(--muted); }
...
<body>
        ...
        <div id="page-dashboard">
            <div class="grid-system">
                <div class="card">
                    ...
                    <div class="sub-box-4">
                        <div class="badge"><span class="badge-label">Today High</span><span id="mx" class="badge-val">--</span></div>
                        <div class="badge"><span class="badge-label">Today Low</span><span id="mn" class="badge-val">--</span></div>
                    </div>
                </div>
                <div class="card">
    ...
    <script>
        ...
        document.getElementById('btn-light').onclick = () => { currentMode = 'light'; applyTheme(); };
        document.getElementById('btn-dark').onclick = () => { currentMode = 'dark'; applyTheme(); };
        document.getElementById('btn-auto').onclick = () => { currentMode = 'auto'; applyTheme(); };
        ...
        async function fetchMonthlySummary() {
            ...
            for (const [month, days] of Object.entries(groups)) {
                html += '<h3>' + month + '</h3><table class="summary-table">' + days.map(d => '<tr><td>' + d.record_date + '</td><td>' + d.max_temp_c + '°C</td><td>' + d.max_wind_kmh + ' km/h</td></tr>').join('') + '</table>';
            }
            document.getElementById('summary-content').innerHTML = html;
        }

        applyTheme(); update(); setInterval(update, 45000);
    </script>
</body>
</html>
    `);
