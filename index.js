...
app.get("/weather", async (req, res) => {
    ...
    try {
        ...
        history.push({
            ...
            dewpt: obs.metric.dewpt || 0
        });

        if (history.length > 48) history.shift(); // last ~48 points
...
async function loadData() {
    try {
        ...
        // Convert GMT to IST for chart X-axis
        const labels=data.history.map(h=>{
            const date=new Date();
            const [hours,minutes]=h.time.split(':').map(Number);
