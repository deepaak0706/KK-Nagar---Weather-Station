...
// History and rain tracking
let todayHistory = [];
let todayMaxRainRate = 0;
let currentDate = new Date().toDateString();

// Weekly/Monthly/Yearly rain totals
let weeklyRain = 0;
...
app.get("/weather", async (req, res) => {
  ...
  try {
    ...
    if (rainRate > todayMaxRainRate) todayMaxRainRate = Math.max(todayMaxRainRate, rainRate);
    ...
    cachedData = {
      ...
      history: todayHistory.map(h => ({ ...h, time: h.time.toISOString() })),
...
h1{text-align:center;padding:16px;margin:0;background:rgba(15,23,42,0.85);}
.status{text-align:center;font-size:12px;padding:6px;opacity:0.85;}
.container{max-width:1100px;margin:0 auto;padding:12px;display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:12px;}
...
.wind-arrow{font-size:36px;margin:8px 0;transition:transform 0.6s;}
.cool{color:#67e8f9;}
.mild{color:#fcd34d;}
.hot{color:#fb923c;}
...
<div class="card">
<div class="grid">
<div class="item"><div class="label">🌡️ TEMP</div><div class="value" id="temp"></div></div>
...
<div class="card">
<div class="grid">
<div class="item"><div class="label">💨 WIND SPEED</div><div class="value" id="wind"></div><div class="wind-arrow" id="arrow">⬆️</div><div class="label" id="winddir"></div></div>
...
<div class="card">
<h3 style="text-align:center;opacity:0.85;">Recent Trends</h3>
...
function createCharts(){
  ...
  charts.temp=new Chart(document.getElementById('tempChart'),{type:'line',data:{labels:[],datasets:[{label:'Temp (°C)',data:[],borderColor:'#67e8f9',backgroundColor:'rgba(103,232,249,0.2)',fill:true,tension:0.3}]},options:opt});
  charts.hum=new Chart(document.getElementById('humChart'),{type:'line',data:{labels:[],datasets:[{label:'Humidity (%)',data:[],borderColor:'#4ade80',backgroundColor:'rgba(74,222,128,0.2)',fill:true,tension:0.3}]},options:opt});
  charts.wind=new Chart(document.getElementById('windChart'),{type:'line',data:{labels:[],datasets:[{label:'Wind (km/h)',data:[],borderColor:'#fb923c',backgroundColor:'rgba(251,146,60,0.2)',fill:true,tension:0.3}]},options:opt});
}
...
async function loadData(){
  try{
    ...
    const d=data.obs; const nowTime=Date.now(); const currentRain=d.metric.precipTotal||0;
