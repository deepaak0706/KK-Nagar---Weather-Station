...
async function syncWithEcowitt() {
    ...
    try {
        ...
        // CRITICAL FIX: Add check for json.data
        ...
        if (now - state.lastDbWrite > 120000) {
            ...
            } catch (dbE) { console.error("DB Write Error:", dbE); }
        }
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; -webkit-font-smoothing: antialiased; }
        ...
        .live-text { font-family: monospace; font-size: 13px; font-weight: 800; color: #22c55e; }
        ...
        .glow-wind { border-color: rgba(251, 191, 36, 0.4); box-shadow: 0 0 30px rgba(251, 191, 36, 0.1); }
        .glow-rain { border-color: rgba(129, 140, 248, 0.4); box-shadow: 0 0 30px rgba(129, 140, 248, 0.1); }
        .glow-uv { border-color: rgba(251, 113, 133, 0.4); box-shadow: 0 0 30px rgba(251, 113, 133, 0.1); }
...
<body>
    <div class="container">
        <div class="header">
            <div><h1>Kk Nagar Weather Hub</h1><div class="live-container"><div class="dot"></div><span class="live-text">LIVE</span><span class="timestamp" id="ts">--:--:--</span></div></div>
        </div>
    ...
    <script>
        ...
        const syncPlugin = {
            ...
            afterDraw: (chart) => {
                if (chart.tooltip?._active?.length) {
                    ...
                    ctx.lineWidth = 1; ctx.strokeStyle = 'rgba(255, 255, 255, 0.2)'; ctx.setLineDash([5, 5]);
                    ctx.stroke(); ctx.restore();
                    Object.values(charts).forEach(oc => {
                        if (oc !== chart) {
                            const pts = oc.getDatasetMeta(0).data;
                            const idx = chart.tooltip.dataPoints[0].index;
                            if (pts[idx]) { oc.tooltip.setActiveElements([{ datasetIndex: 0, index: idx }], { x: pts[idx].x, y: pts[idx].y }); oc.draw(); }
                        }
                    });
                }
            }
        ...
        function setupChart(id, label, col, minZero = false) {
            ...
            return new Chart(ctx, {
                ...
                options: { 
                    ...
                    scales: { x: { ticks: { color: '#94a3b8' }, grid: { display: false } }, y: { beginAtZero: minZero, ticks: { color: '#94a3b8' }, grid: { color: 'rgba(255,255,255,0.03)' } } }
                }
            });
        }
        ...
        async function update() {
            try {
                ...
                const trendVal = d.temp?.trend || 0;
                const tIcon = trendVal > 0 ? '↗' : trendVal < 0 ? '↘' : '→';
                const tCol = trendVal > 0 ? 'var(--max-t)' : trendVal < 0 ? '#22c55e' : '#94a3b8';
                document.getElementById('tr').innerHTML = '<span style="color:'+tCol+'">'+tIcon+' '+Math.abs(trendVal)+'°C/hr Trend</span>';
                ...
                const getTrend = (val) => val > 0 ? {i:'▲', c:'#10b981'} : val < 0 ? {i:'▼', c:'#fb7185'} : {i:'●', c:'#475569'};
                const hT = getTrend(d.atmo?.hTrend || 0);
                ...
                const pT = getTrend(d.atmo?.pTrend || 0);
                ...
                document.getElementById('mw').innerText = (d.wind?.maxS || '0') + ' km/h';
                document.getElementById('mg').innerText = (d.wind?.maxG || '0') + ' km/h';
                ...
                document.body.classList.toggle('solar-low', (d.solar?.rad || 0) <= 0);
                document.getElementById('card-wind').classList.toggle('glow-wind', (d.wind?.speed || 0) > 15);
                document.getElementById('card-rain').classList.toggle('glow-rain', (d.rain?.rate || 0) > 0);
                document.getElementById('card-atmo').classList.toggle('glow-uv', (d.solar?.uvi || 0) > 5);

                const rStat = (d.rain?.rate || 0) > 0 ? {t:'Raining', c:'#38bdf8', b:'rgba(56,189,248,0.1)'} : {t:'Dry', c:'#64748b', b:'rgba(255,255,255,0.06)'};
                ...
                const syncDate = d.lastSync ? new Date(d.lastSync) : new Date();
