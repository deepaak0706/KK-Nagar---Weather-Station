    ...
    try {
        ...
        if (state.todayHistory.length > 0) {
            ...
            if (pastRecord && dailyRain > pastRecord.rainTotal) {
                ...
                const lastRainCheck = now - 150000; // 2.5 Min Timeout
    ...
    <style>
        ...
        * { box-sizing: border-box; -webkit-font-smoothing: antialiased; }
        body { 
            ...
            background: #020617; color: #f8fafc; padding: 40px 24px; min-height: 100vh;
        ...
        .grid-system { display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: var(--gap); margin-bottom: var(--gap); }
        ...
        /* DYNAMIC RAIN PULSE */
        @keyframes rainGlow { 
            0% { border-color: var(--border); box-shadow: 0 25px 50px -12px rgba(0,0,0,0.5); }
            50% { border-color: var(--rain); box-shadow: 0 0 30px rgba(99, 102, 241, 0.3); }
            100% { border-color: var(--border); box-shadow: 0 25px 50px -12px rgba(0,0,0,0.5); }
        }
        .is-raining { animation: rainGlow 3s infinite; }

        .icon-box { margin-bottom: 20px; }
        .icon-box svg { width: 32px; height: 32px; stroke: currentColor; stroke-width: 2.5; fill: none; transition: transform 1.5s ease; }
...
<body>
    <div class="container">
        ...
        <div class="grid-system">
            ...
            <div class="card">
                <div class="icon-box" style="color:var(--wind)">
                    <svg id="comp" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><circle cx="12" cy="12" r="10"/><path d="m12 16-3-4h6l-3 4Z" fill="currentColor"/><path d="M12 8v4"/></svg>
                </div>
            ...
            <div class="card" id="rain_c">
                ...
                <div id="rr" style="font-size: 13px; font-weight: 700; color:var(--rain)">Rate: 0.0 mm/h</div>
                <div class="sub-box">
                    <div class="badge"><span class="badge-label">Max Rate</span><span id="mr" class="badge-val">--</span></div>
                    <div class="badge"><span class="badge-label">Status</span><span id="rs" class="badge-val">Dry</span></div>
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        async function update() {
            try {
                ...
                // --- DYNAMIC VISUALS ---
                // Compass Rotation
                document.getElementById('comp').style.transform = 'rotate(' + d.wind.deg + 'deg)';
                
                // Rain Card Glow
                document.getElementById('rain_c').classList.toggle('is-raining', d.rain.rate > 0);
                
                // Day/Night Background
                document.body.style.background = d.solar.rad < 5 ? '#02040a' : '#020617';
                
                document.getElementById('ts').innerText = new Date(d.lastSync).toLocaleTimeString('en-IN', { hour12: false });

                // Graphs
                if (!charts.cT) {
                    const cfg = (id, label, col) => new Chart(document.getElementById(id), { type:'line', data:{labels:[], datasets:[{label:label, data:[], borderColor:col, tension:0.4, pointRadius:0, fill:true, backgroundColor:col+'11'}]}, options:{maintainAspectRatio:false, plugins:{legend:{labels:{color:'#fff'}}}, scales:{x:{display:false},y:{grid:{color:'rgba(255,255,255,0.03)'},ticks:{color:'#64748b'}}}}});
                    charts.cT = cfg('cT', 'Temperature Trend', '#38bdf8');
                    charts.cR = cfg('cR', 'Rain Rate Intensity', '#6366f1');
                }
