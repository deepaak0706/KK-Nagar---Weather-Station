...
async function syncWithEcowitt() {
    ...
    // If state is fresh/reset, pull today's peaks from the DB
    ...
    <style>
        ...
        * { box-sizing: border-box; -webkit-font-smoothing: antialiased; }
        ...
        .card, .graph-card { 
            ...
            transition: transform 0.3s ease; animation: fade-in-up 0.6s ease-out forwards; opacity: 0;
        }
        ...
        .glow-wind { border-color: rgba(251, 191, 36, 0.4); }
        .glow-rain { border-color: rgba(129, 140, 248, 0.4); }
        .glow-uv { border-color: rgba(251, 113, 133, 0.4); }
    ...
    <script>
        ...
        function setupChart(id, label, col, minZero = false) {
            ...
            return new Chart(ctx, {
                ...
                options: { 
                    ...
                    scales: { x: { grid: { display: false } }, y: { beginAtZero: minZero, grid: { color: 'rgba(255,255,255,0.05)' } } }
                }
            });
        }
        ...
        async function update() {
            try {
                ...
                document.getElementById('tr').innerText = tIcon + ' ' + Math.abs(d.temp.trend) + '°C/hr Trend';
