...
async function syncWithEcowitt() {
    ...
    try {
        ...
        // FIX: FETCH STATS + TIMESTAMPS FROM DB (Resetting at Midnight IST)
        const statsRes = await pool.query(`
            WITH daily_data AS (
                SELECT *, (time AT TIME ZONE 'UTC' AT TIME ZONE 'Asia/Kolkata') as local_time
                FROM weather_history 
                WHERE (time AT TIME ZONE 'UTC' AT TIME ZONE 'Asia/Kolkata')::date = CURRENT_DATE
            )
            SELECT 
                MAX(temp_f) as max_t, MIN(temp_f) as min_t,
                MAX(wind_speed_mph) as max_w, MAX(wind_gust_mph) as max_g, MAX(rain_rate_in) as max_rr,
                (SELECT TO_CHAR(local_time, 'HH24:MI:SS') FROM daily_data WHERE temp_f = (SELECT MAX(temp_f) FROM daily_data) ORDER BY local_time DESC LIMIT 1) as max_t_time,
                (SELECT TO_CHAR(local_time, 'HH24:MI:SS') FROM daily_data WHERE temp_f = (SELECT MIN(temp_f) FROM daily_data) ORDER BY local_time ASC LIMIT 1) as min_t_time,
                (SELECT TO_CHAR(local_time, 'HH24:MI:SS') FROM daily_data WHERE wind_speed_mph = (SELECT MAX(wind_speed_mph) FROM daily_data) ORDER BY local_time DESC LIMIT 1) as max_w_time,
                (SELECT TO_CHAR(local_time, 'HH24:MI:SS') FROM daily_data WHERE wind_gust_mph = (SELECT MAX(wind_gust_mph) FROM daily_data) ORDER BY local_time DESC LIMIT 1) as max_g_time
            FROM daily_data
        `);

        if (statsRes.rows.length > 0 && statsRes.rows[0].max_t !== null) {
            const r = statsRes.rows[0];
            state.maxTemp = parseFloat(((r.max_t - 32) * 5 / 9).toFixed(1)); state.maxTempTime = r.max_t_time;
            state.minTemp = parseFloat(((r.min_t - 32) * 5 / 9).toFixed(1)); state.minTempTime = r.min_t_time;
            state.maxWindSpeed = parseFloat((r.max_w * 1.60934).toFixed(1)); state.maxWindTime = r.max_w_time;
            state.maxGust = parseFloat((r.max_g * 1.60934).toFixed(1)); state.maxGustTime = r.max_g_time;
            state.maxRainRate = parseFloat(r.max_rr || 0);
        }

        // FIX: GRAPH X-AXIS FORMATTING
        const historyRes = await pool.query(`
            SELECT 
                TO_CHAR(time AT TIME ZONE 'UTC' AT TIME ZONE 'Asia/Kolkata', 'YYYY-MM-DD"T"HH24:MI:SS') as ist_time, 
...
// FIX: BACKGROUND DAEMON
setInterval(() => { syncWithEcowitt().catch(e => console.log(e)); }, 60000);
    ...
    <div class="container">
        ...
        <div class="grid-system">
            ...
            <div class="card" id="card-rain" style="animation-delay:0.4s">
                ...
                <div class="sub-box-4" style="grid-template-columns:1fr"><div class="badge"><span class="badge-label">Max Rate</span><span id="mr" class="badge-val" style="color:var(--rain)">--</span></div></div>
            </div>
        </div>
        <div class="grid-system">
    ...
    <script>
        ...
        function setupChart(id, label, col) {
            ...
            return new Chart(ctx, {
                ...
                options: { 
                    ...
                    plugins: { legend: { labels: { color: '#f8fafc', font: { family: 'Outfit', weight: '700' } } } },
        ...
        async function update() {
            try {
                ...
                const rs = document.getElementById('rain_status'); rs.innerText = rStat.t; rs.style.color = rStat.c; rs.style.background = rStat.b;
