...
app.get("/weather", async (req, res) => {
    ...
    try {
        ...
        // TEMP RATE per hour using last full hour comparison
        ...
        // Daily max/min
        if (todayMaxTemp === null || parseFloat(tempC) > todayMaxTemp) todayMaxTemp = parseFloat(tempC);
        if (todayMinTemp === null || parseFloat(tempC) < todayMinTemp) todayMinTemp = parseFloat(tempC);
    ...
    <div class="grid">
        <div class="item">
            ...
            <div style="font-size:13px; margin-top:4px;">
                Max: <span id="maxTemp" class="maxval"></span> | Min: <span id="minTemp" class="minval"></span>
            </div>
        </div>
        <div class="item">
...
<div class="card grid">
    <div class="item">
        ...
        <div class="label" style="font-size:13px;">Max Speed: <span id="maxWind" class="maxval"></span></div>
    </div>
    <div class="item">
        <div class="label">GUST</div>
        <div class="value" id="gust"></div>
        <div class="label" style="font-size:13px;">Max Gust: <span id="maxGust" class="maxval"></span></div>
    </div>
    <div class="item">
...
function createCharts(){
    const opt = { animation:false, responsive:true, scales:{y:{beginAtZero:false, ticks:{callback:(v)=>parseFloat(v).toFixed(1)}}} };
    ...
    charts.rain = new Chart(document.getElementById('rainChart'), { type:'line', data:{labels:[], datasets:[{label:'Rain Rate (mm/hr)', data:[], borderColor:'#fcd34d', tension:0.3}]}, options:opt });
}
...
async function loadData(){
    try{
        ...
        document.getElementById('maxTemp').innerText=o.maxTemp;
        document.getElementById('minTemp').innerText=o.minTemp;
        ...
        document.getElementById('status').innerHTML='✅ Live from Ecowitt • Updated '+new Date().toLocaleTimeString('en-IN');

        // IST X-axis for charts
...
app.listen(PORT,()=>{console.log("✅ KK Nagar Weather Station (Ecowitt) running on port "+PORT+" • Refresh interval: 30s");});
