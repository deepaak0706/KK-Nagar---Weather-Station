...
async function syncWithEcowitt() {
    ...
    try {
        ...
        // FIXED TREND LOGIC: Looks back at the earliest available record in the last 30-60 mins
        let trend = 0;
        if (state.todayHistory.length > 5) {
            const lookback = Math.min(state.todayHistory.length - 1, 45); // Compare with ~45 mins ago
            const pastTemp = state.todayHistory[state.todayHistory.length - 1 - lookback].temp;
            trend = parseFloat((tempC - pastTemp).toFixed(2)); 
        }
    ...
    <script>
        ...
        async function update() {
            try {
                ...
                document.getElementById('tr').innerHTML = \`<span style="color:\${trendCol}">\${trendIcon} \${Math.abs(d.temp.trend)}°C Trend</span>\`;
