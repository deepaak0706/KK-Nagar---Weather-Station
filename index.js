...
let cachedData = null;
let lastFetch = 0;
let todayHistory = [];           // Stores today's data points
...
app.get("/weather", async (req, res) => {
    ...
    try {
        ...
        if (todayHistory.length > 0) {
            ...
            const lastEntryTime = new Date(currentDate + ' ' + lastEntry.time).getTime();
            const timeDiff = (Date.now() - lastEntryTime) / 1000;
            if (timeDiff > 0) {
                rainRate = ((rainTotal - lastEntry.rain) * 3600) / timeDiff;
            }
        }

        if (rainRate > todayMaxRainRate) todayMaxRainRate = Math.max(rainRate, todayMaxRainRate);
        ...
        if (todayHistory.length > 1440) todayHistory.shift(); // Max ~1 day

        cachedData = {
            obs,
            sunrise: sunData.results?.sunrise || null,
            sunset: sunData.results?.sunset || null,
...
async function loadData() {
    try {
        ...
        const maxRR = data.maxRainRate ? ' (' + format(data.maxRainRate) + ' mm/hr Max)' : '';
        ...
        // Charts
        const labels = data.history.map(h => h.time); // now local IST
