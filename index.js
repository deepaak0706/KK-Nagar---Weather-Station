...
app.get("/weather", async (req, res) => {
    ...
    try {
        ...
        const rainRateMmHr = parseFloat((parseFloat(d.rainfall.rain_rate.value) * 25.4).toFixed(2));
        ...
        // Temperature change per hour based on previous hour
        let tempChangeRate = 0;
        if (lastTemp !== null) {
            const diffHours = (Date.now() - lastTempTime) / (1000 * 3600);
            if (diffHours > 0) tempChangeRate = parseFloat(((tempC - lastTemp) / diffHours).toFixed(1));
        }
        lastTemp = tempC;
        ...
        const currentTimeIST = new Date().toLocaleTimeString('en-IN', {hour12: false, hour:'2-digit', minute:'2-digit', timeZone:'Asia/Kolkata'});
        ...
        cachedData = {
            outdoor: { temp: tempC, feelsLike: feelsLikeC, dewPoint: dewPointC, humidity: d.outdoor.humidity.value, tempChangeRate },
            rainfall: { rainRate: rainRateMmHr, totalRain: totalRainMm, maxRainRate: todayMaxRainRate },
            wind: { speed: windSpeedKmh, gust: windGustKmh, maxSpeed: todayMaxWind, maxGust: todayMaxGust, direction: d.wind.wind_direction.value },
        ...
        res.status(500).json({ error: "Failed to fetch Ecowitt data" });
    }
});
...
<div style="font-size:14px; margin-top:4px;">Max: <span id="tempMax"></span> | Min: <span id="tempMin"></span></div>
</div>
<div class="item">
...
<div style="font-size:13px;">Max Speed: <span id="windMax"></span></div>
</div>
<div class="item">
<div class="label">WIND GUST</div>
<div class="value" id="gust"></div>
<div style="font-size:13px;">Max Gust: <span id="gustMax"></span></div>
</div>
...
<div class="card">
<h3 style="text-align:center; opacity:0.9;">Recent Trends</h3>
...
const options = {animation:false, scales:{y:{beginAtZero:true, ticks:{precision:1}}}};
charts.temp = new Chart(document.getElementById('tempChart'), {type:'line', data:{labels:[], datasets:[{label:'Temperature (°C)', data:[], borderColor:'#67e8f9', tension:0.3}]}, options});
charts.hum = new Chart(document.getElementById('humChart'), {type:'line', data:{labels:[], datasets:[{label:'Humidity (%)', data:[], borderColor:'#4ade80', tension:0.3}]}, options});
charts.wind = new Chart(document.getElementById('windChart'), {type:'line', data:{labels:[], datasets:[{label:'Wind Speed (km/h)', data:[], borderColor:'#fb923c', tension:0.3}]}, options});
charts.rain = new Chart(document.getElementById('rainChart'), {type:'line', data:{labels:[], datasets:[{label:'Rain Rate (mm/hr)', data:[], borderColor:'#facc15', tension:0.3}]}, options});
}
...
document.getElementById('tempRate').innerHTML=o.tempChangeRate>=0?'↑ '+o.tempChangeRate+' °C/hr':'↓ '+Math.abs(o.tempChangeRate)+' °C/hr';
...
document.getElementById('windMax').innerText=w.maxSpeed+' km/h';
document.getElementById('gustMax').innerText=w.maxGust+' km/h';
...
document.getElementById('status').innerText='✅ Live from Ecowitt • Updated '+new Date().toLocaleTimeString('en-IN', {timeZone:'Asia/Kolkata'});
...
app.listen(PORT, ()=>console.log("✅ KK Nagar Weather Station running on port "+PORT+" | Refresh every 30 sec"));
