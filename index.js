...
async function syncWithEcowitt() {
    ...
    try {
        ...
        if (parseFloat((d.wind.wind_speed.value * 1.60934)) > state.maxWindSpeed) state.maxWindSpeed = parseFloat((d.wind.wind_speed.value * 1.60934).toFixed(1));
        if (parseFloat((d.wind.wind_gust.value * 1.60934)) > state.maxGust) state.maxGust = parseFloat((d.wind.wind_gust.value * 1.60934).toFixed(1));
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; -webkit-font-smoothing: antialiased; }
        ...
        .card, .graph-card { background: var(--card); padding: 32px; border-radius: 28px; border: 1px solid var(--border); backdrop-filter: blur(24px); box-shadow: 0 24px 40px -10px rgba(0, 0, 0, 0.4); position: relative; }
        .label { color: #94a3b8; font-size: 13px; font-weight: 800; text-transform: uppercase; letter-spacing: 2px; }
        .main-val { font-size: 56px; font-weight: 900; margin: 4px 0; display: flex; align-items: baseline; }
        ...
        /* Modern Trendy Trend Badge */
        .trend-badge { 
            font-size: 13px; font-weight: 800; margin-bottom: 24px; display: inline-flex; 
            align-items: center; gap: 6px; padding: 6px 12px; background: rgba(255,255,255,0.06); 
            border-radius: 10px; border: 1px solid rgba(255,255,255,0.1); 
        }
        .trend-up { color: #fb7185; } .trend-down { color: #34d399; }
        ...
        .badge-val { font-size: 16px; font-weight: 700; color: #f1f5f9; display: flex; align-items: center; }
        .time-mark { font-size: 10px; font-weight: 800; color: #94a3b8; background: rgba(255,255,255,0.08); padding: 2px 6px; border-radius: 6px; margin-left: 4px; border: 1px solid rgba(255,255,255,0.1); }
    ...
    <div class="container">
        <div class="header">
            ...
            <div class="live-container">
                <div class="dot"></div><span id="ts" class="timestamp">--:--:--</span>
            </div>
        </div>
        <div class="grid-system">
            ...
            <div class="card">
                ...
                <div class="minor-line"><span id="rr_main" style="color:var(--rain)">Rate: --</span></div>
    ...
    <script>
        ...
        function setupChart(id, label, col, minZero = false) {
            ...
            return new Chart(ctx, {
                ...
                options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: true, labels: {color:'#fff'} } }, scales: { x: { grid: { display: false } }, y: { beginAtZero: minZero, grid: { color: 'rgba(255,255,255,0.05)' } } } }
            });
        }
        ...
        async function update() {
            try {
                ...
                // Modernized Trend Icon Only
                const tIcon = d.temp.trend > 0 ? '<span class="trend-up">▲</span>' : d.temp.trend < 0 ? '<span class="trend-down">▼</span>' : '●';
                document.getElementById('tr').innerHTML = tIcon + ' ' + Math.abs(d.temp.trend) + '°C/hr';
                ...
                // Hum/Pres Indicators
                const hIcon = d.atmo.hTrend > 0 ? '<span style="color:#fb7185">▲</span>' : d.atmo.hTrend < 0 ? '<span style="color:#34d399">▼</span>' : '';
                document.getElementById('h_tr').innerHTML = hIcon;
                const pIcon = d.atmo.pTrend > 0 ? '<span style="color:#34d399">▲</span>' : d.atmo.pTrend < 0 ? '<span style="color:#fb7185">▼</span>' : '';
                document.getElementById('p_tr').innerHTML = pIcon;
